...
1. **Dependencia de diccionario estático**: STEM_MAP es definido manualmente y no cubre todos los dominios. Parcialmente mitigado por la expansión por tags (v3.1), que no requiere STEM_MAP para encontrar relaciones si los datos están correctamente taggeados.
2. **Falsos positivos vectoriales**: Consultas genéricas producen ruido semántico en memoria_vectorial.
3. **Escalabilidad (parcialmente resuelto en v3.4)**: ILIKE ANY escanea secuencialmente; no escala a millones de registros. Los índices GIN con pg_trgm (Sección 8.5) aceleran la búsqueda textual mediante trigramas, pero el planificador de PostgreSQL puede optar por escaneo secuencial en tablas pequeñas (<10⁴ registros). Para conjuntos mayores (>10⁵ registros), se recomienda verificar que el planificador use los índices GIN.
4. **Sin normalización de texto**: Tildes y caracteres especiales pueden afectar la coincidencia ILIKE.
6. **Tags inconsistentes (parcialmente resuelto en v3.3)**: La expansión por tags depende de que los datos estén correctamente taggeados al persistir. Datos sin tags no se benefician de esta fase. Auto-Tagging v1.0 (Sección 8.3) mitiga este problema asignando tags automáticamente, pero solo aplica a datos nuevos; registros preexistentes sin tags quedan fuera.

### 8.2 Trabajo Futuro

*(En este momento no hay trabajo futuro identificado. Ver Avances Recientes en Sección 8.7.)*
