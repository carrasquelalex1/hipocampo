...
Hipocampo is not a sticky note. It is an **external brain** with 30 MCP tools. **Use all of them, not just save/search.**
...
Advanced client-side usage patterns. No server changes required — these leverage existing tools in new ways.

### Confidence Heuristics (v1)

When evaluating `search_hipocampo` results, apply an implicit weight:
- `nivel=automatica` → weight 1.0
- `nivel=semantica` → weight 0.8
- `nivel=episodica` + accessed recently (<30d) → weight 0.7
- `nivel=episodica` + no access + >90d → weight 0.4

For critical decisions (deploy, delete, edit in production): use only memories with weight ≥ 0.7. This is a volatile calculation — not stored, applied when receiving results.

### Semantic Drift Detection (v1)

Before `save_hipocampo` with `memory_type="decision"`:
1. Run `search_hipocampo` with the new content
2. If a result has high implicit similarity (>0.7) AND contradictory content:
   - Do not save as new
   - Use `update_hipocampo` on the existing memory
   - Create `link_hipocampo(type="contradicts")` between both
3. If the existing memory is >180 days old: add a "possible drift" note in the `extra` field

Contradictions are detected not by date, but by **explicit contradiction** in content.

### Graph Reasoning (v1)

For error diagnosis or complex decisions:
1. `search_hipocampo(query)` → seed nodes
2. For each node: `graph_hipocampo(node_id, depth=2)`
3. Identify "bridge" nodes (appear in multiple subgraphs)
4. `path_hipocampo(from=root_cause, to=effect)` → causal chain
5. Respond with the chain + context from each node

Useful for: root-cause debugging, analyzing past decisions, regression audits.
